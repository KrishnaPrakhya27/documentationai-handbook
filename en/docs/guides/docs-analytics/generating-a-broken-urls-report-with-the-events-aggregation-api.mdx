---
title: Generating a broken URLs report with the Events Aggregation API
description: Export broken URL activity from GitBook analytics with curl and jq to build a practical monthly report.
metaTitle: Generating a broken URLs report with the Events Aggregation API | Guides | GitBook Documentation
...
Broken URLs appear in [Site analytics](/en/docs/analytics/insights).

This guide shows how to export the same signal with the [Events Aggregation API](/en/docs/guides/docs-analytics/track-advanced-analytics-with-gitbooks-events-aggregation-api).
...
<Steps>
  <Step title="Set your local variables" titleType="h3">
  ...
  <Step title="Query broken URL events for the last 30 days" titleType="h3">
  ...
  <Step title="Turn the response into a readable table with jq" titleType="h3">
...
If you want to go deeper on query structure, filters, and other reporting patterns, see [Track advanced analytics with GitBook's Events Aggregation API](/en/docs/guides/docs-analytics/track-advanced-analytics-with-gitbooks-events-aggregation-api).
