...
<Steps>
  ...
  <Step title="Create an iframe">
    ...
    <Callout kind="info">
      If you use the Assistant tab, set `allow="clipboard-write"` on the iframe. The NPM package leaves iframe setup to you. The standalone [Script tag](/en/docs/publish/embedding/implementation/script) implementation adds this automatically.
    </Callout>
  </Step>

  <Step title="Attach the frame">
  ...
  <Step title="Control the embed programmatically">
...
Pass to `getFrameURL({ visitor: {...} })`. Used for [Adaptive Content](/en/docs/publish/adaptive-content) and [Authenticated Access](/en/docs/publish/site-audience/authenticated-access).
