...
metaTitle: Script tag | GitBook Documentation
...
<Steps>
  <Step title="Copy your embed script URL" titleType="h3">
  ...
  <Step title="Add the script to your HTML" titleType="h3">
  ...
  <Step title="If your docs require authentication" titleType="h3">
    If your docs [are behind auth](/en/docs/publish/site-audience/authenticated-access), the script must include a signed JWT token.
  ...
  <Step title="Verify" titleType="h3">
    Reload your page.
...
Custom AI tools to extend the Assistant. See [Creating custom tools](/en/docs/publish/embedding/configuration/creating-custom-tools) for details.
...
<Callout kind="info">
  <strong>Note:</strong> This option is only available when using the standalone script tag implementation. For React or Node.js implementations, you'll need to create your own button to trigger the embed.
</Callout>
...
Pass when initializing with `GitBook('init', options, frameOptions)`. Used for [Adaptive Content](/en/docs/publish/adaptive-content) and [Authenticated Access](/en/docs/publish/site-audience/authenticated-access).
...
- **Authenticated docs not accessible** – If your docs require sign-in, you must provide the `visitor.token` when initializing. See [Using with authenticated docs](/en/docs/publish/embedding/using-with-authenticated-docs).
