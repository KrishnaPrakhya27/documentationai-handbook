---
title: Connect to custom tools
description: Connect GitBook Assistant to any tool you can call from your app — especially support workflows
metaTitle: Connect to custom tools | GitBook Documentation
...
Custom tools let the GitBook Assistant inside the [Docs Embed](/en/docs/publish/embedding) run real actions.
...
<Callout kind="info">
  In addition to tools you define in the Embed config, the Assistant can also use any [MCP servers you set up](/en/docs/ai-for-your-readers/mcp-servers-for-published-docs) in **Settings → AI & MCP**.
</Callout>
...
- Via `window.GitBook("configure", …)` for the [script tag](/en/docs/publish/embedding/implementation/script) implementation
- Via the `tools` prop for the [Node.js/NPM](/en/docs/publish/embedding/implementation/nodejs) package and [React](/en/docs/publish/embedding/implementation/react) components
...
<Callout kind="info">
  Pair this with an always-visible **Contact Support** action in the embed sidebar. You can configure actions by following [Customizing the Embed](/en/docs/publish/embedding/configuration/customizing-docs-embed).
</Callout>

### Next steps

- Need the full embed API surface? See [API Reference](/en/docs/publish/embedding/configuration/reference).
- Want more UI controls (greeting, suggestions, actions)? See [Customizing the Embed](/en/docs/publish/embedding/configuration/customizing-docs-embed).
